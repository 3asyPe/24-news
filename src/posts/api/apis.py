...
from posts.api.validators import CreateProductSerializer, UpdateProductSerializer
...
def put_product_api(request):
    ...
    return Response({"message": "chenged successfully"}, status=200)

@api_view(["PUT"])
def put_product_api(request):
    data = request.POST or request.data

    validator = UpdateProductSerializer(data=data) #data = request.POST or request.data
    validator.is_valid(raise_exception=True)
